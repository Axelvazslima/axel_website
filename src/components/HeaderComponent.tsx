...
function Header() {
  ...
  return (
    <header className={`flex ${menu ? "justify-end" : "justify-between"} items-center w-screen h-4 bg-[#0F172A] text-gray-200 p-8 font-mono`}>
      ...
      <button className={`${menu ? "block" : "hidden"} bg-inherit shadow-sm shadow-blue-900 border-2 p-2 mr-2 rounded-lg border-gray-100`} onClick={() => setMenu(!menu)}>Close</button>
